...
class DbProvider {
  ...
  static const _dbName = "novelviewer.db";
  ...
  static Future _onCreate(Database db, int version) async {
    await Future.wait([
      for (Tables t in Tables.values) db.execute(_cmd(t)),
    ]);
  }

  static String _cmd(Tables t) {
    // UNOQUE(${EpisodeTable.novelId.name}, ${EpisodeTable.order.name})
    final columns = switch (t.tablename) {
      NovelTable.tablename =>
        NovelTable.values.map((e) => "${e.name} ${e.options}").join(","),
      EpisodeTable.tablename =>
        EpisodeTable.values.map((e) => "${e.name} ${e.options}").join(","),
      ChapterTable.tablename =>
        ChapterTable.values.map((e) => "${e.name} ${e.options}").join(","),
      _ => "",
    };
    final pkey = switch (t.tablename) {
      NovelTable.tablename => "PRIMARY KEY(${NovelTable.pkey})",
      EpisodeTable.tablename => "PRIMARY KEY(${EpisodeTable.pkey})",
      ChapterTable.tablename => "PRIMARY KEY(${ChapterTable.pkey})",
      _ => "",
    };
  ...
  //   sort: "key3"
  // )
  static Future<List<Map<String, dynamic>>> queryAllRows(Tables t,
      {Map<String, dynamic>? filter, String? sort}) async {
    String? statement = filter?.entries.map((e) {

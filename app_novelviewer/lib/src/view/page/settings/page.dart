...
import 'package:app_novelviewer/src/view/page/settings/custom_buttons_dialog.dart';
import 'package:app_novelviewer/src/view/page/settings/default_url_dialog.dart';
...
class _SettingsPageState extends ConsumerState<SettingsPage> {
          ...
          child: ListView(
            children: ListTile.divideTiles(
              ...
              tiles: [
                ...
                // Customize
                ListTile(
                  subtitle: const Text(SettingsPageText.customSection),
                  textColor: Theme.of(context).colorScheme.primary,
                ),

                ListTile(
                  leading: const Icon(Icons.search),
                  title: const Text(SettingsPageText.defaultURL),
                  subtitle: Text(ref.watch(defaultURLProvider)),
                  onTap: _onTapDefaultURL,
                ),
                ListTile(
                  leading: const Icon(Icons.app_registration),
                  title: const Text(SettingsPageText.customButtons),
                  onTap: _onTapCustomButtons,
                ),
                Text(ref.watch(customButtonsProvider).join("\n")),
  ...
  void _onTapDefaultURL() async {
    final String? res = await showDialog(
      context: context,
      builder: (_) => DefaultUrlDialog(url: ref.watch(defaultURLProvider)),
    );
    if (res == null || res.isEmpty) {
      return;
    }
    ref.read(defaultURLProvider.notifier).set(res);
  }

  void _onTapCustomButtons() async {
    final List<String>? res = await showDialog(
      context: context,
      builder: (_) =>
          CustomButtonsDialog(buttons: ref.watch(customButtonsProvider)),
    );
    if (res == null || res.isEmpty) {
      return;
    }
    ref.read(customButtonsProvider.notifier).set(["www"]);
    // ref.read(customButtonsProvider.notifier).set(res);
  }
